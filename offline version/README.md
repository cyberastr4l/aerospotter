# AeroSpotter: Talent Show Edition (Offline Standalone)

**AeroSpotter (Offline Standalone)** is a bold, high-contrast aircraft and airport identification quiz game designed in a clean, modern **Runway Brutalist** style. This standalone edition is fully self-contained and pre-configured to run entirely offline without requiring any internet connection.

It is designed to be played live with a host presenting the screen to an audience.

---

## 🎮 How to Play

1. **Launch the Game**: Open `index.html` in any web browser and click **Launch Game** (or **Iniciar Jogo**) on the welcome screen. The deck will automatically shuffle a random sequence of aircraft and airports.
...
*   **100% Offline Standalone**: Loads all assets (aircraft silhouettes and airport code images) locally from the bundled `planes/` and `airports/` directories. No external CDN streams, network calls, or API connections are required.
