...
function setupRound(roundIdx) {
    ...
    DOM.planeImage.src = currentPlane.filename;
    DOM.planeImage.classList.remove('blurred');
    if (isAirport) {
        DOM.codeImage.src = currentPlane.codeFilename;
    ...
    state.roundData.forEach((plane, idx) => {
        ...
        item.innerHTML = `
            <div class="debrief-thumb-container">
                <img src="${plane.filename}" class="debrief-thumb" alt="${displayName}">
            </div>
